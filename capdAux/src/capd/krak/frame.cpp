...
/**
  Moves the left top corner to (\c arglti, \c argltj) and the left botton
  corner to (\c argrbi, \c argrbj) where the coordinates are given in
...
/**
  Moves the left top corner to \c lt and the left botton
  corner to \c rb where the coordinates are given in
...
/**
  Generates a new frame as a subframe of \c prntFrm
  with left top corner (\c lti,\c ltj),right bottom corner at (\c rbi,\c rbj),
...
/**
  Generates a new frame as a subframe of \c prntFrm
  with left top corner \c lt, right bottom corner at \c rb,
...
/**
  ...
  with left top corner (\c lti,\c ltj), right bottom corner at
  (\c rbi,\c rbj). \c bgc and \c fgc are the background and foreground color,
  \c im and \c jm are the margins.
*/
...
/**
  Generates a new frame as a subframe of \c prntFrm and sets the world
  coordinates and colors
@param the_lti,the_ltj The coordinates of the left top corner in percentage of the
   parent frame
@param the_rbi,the_rbj The coordinates of the right botton corner in percentage of
   the parent frame
@param the_swx,the_swy,the_nex,the_ney The world coordinates. Check setWorldCoord for details
@param the_bgc,the_fgc The background and foreground color
@param the_im,the_jm The margins for text in device coordinates
*/
...
/**
  draws a line from the current position to (\c i, \c j)
  using the color \c color. If color is FRAME_FG then the
...
/**
  draws the text given by \c t from the current position
  using the color \c color. If color is FRAME_FG then the
  frame foreground color is used.
*/
...
/**
  Draw a dot at position (\c i, \c j) (in pixel coordinates)
  and color \c color
*/
...
/**
  Draw a dot at position (\c x, \c y) (in world coordinates)
  and color \c color
*/
...
/**
  Draw a circle at position (\c i, \c j) of radius \c r (in pixel coordinates)
  and color \c color
*/
...
/**
  Draw a circle at position (\c x, \c y) (in world coordinates)
  of radius \c r (in pixel coordinates) and color \c color
*/
...
/**
  Draws a line from the point (\c x1, \c y1) to (\c x2, \c y2) (in pixel
  coordinates)in the color \c color. The current position is moved to the
  end of the line
*/
...
/**
  Draws a line from the point (\c x1, \c y1) to (\c x2, \c y2) (in world
  coordinates)in the color \c color. The current position is moved to the
  end of the line
*/
...
/**
  Draws a polygon given by \c nPoints pairs of the coordinates
  in the table \c coords in the color i@(color)
*/
...
/**
  Draws a polygon given by \c nPoints pairs of the world coordinates
  in the table \c coords in the color i@(color)
*/
...
/**
  Fills a polygon given by \c nPoints pairs of the coordinates
  in the table \c coords in the color i@(color)
*/
...
/**
  Fills a polygon given by \c nPoints pairs of the world coordinates
  in the table \c coords in the color i@(color)
*/
...
/**
  ...
  (\c lti,\c ltj) and (\c rbi,\c rbj). The arc is indicated by the points
  (\c bi,\c bj) and (\c ei,\c ej). The arc is drawn in the color i@(color)
*/
...
/**
  Draw a cross at position (\c x, \c y) (in world coordinates),
  color \c color and size \c size
*/
...
/**
  Prints the character \c c at the current position
*/
...
/**
  Prints the number \c n at the current position
*/
capd::krak::Frame &capd::krak::Frame::operator<<(long n)
{
   std::ostringstream bufstream;
   bufstream << n << '\0';
   capd::krak::DrawTxtc(this,bufstream.str().c_str());
   return *this;
}
...
/**
  Outputs the string \c a_string on the frame.
*/
...
/**
  Outputs the string \c text on the frame.
*/
