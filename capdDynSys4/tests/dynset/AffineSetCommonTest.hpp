...
BOOST_AUTO_TEST_CASE(test_getset_x) {
  SetType set(SetType::VectorType(2));
...
BOOST_AUTO_TEST_CASE(test_max_width){
  SetType::VectorType v(SetType::VectorType(2));
  v[0] = SetType::ScalarType(-1,1); v[1] = SetType::ScalarType(1,3);
  SetType S(v);
  BOOST_CHECK_EQUAL(2, maxWidth(SetType::VectorType(S)));
