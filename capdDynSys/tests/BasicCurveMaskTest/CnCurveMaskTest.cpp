...
{
  check(m.begin(),c.begin(),c.end());
}


void doTest(DCnTimeMap& tm, const BJet& mask){
  DVector u(1,3,1);
  DJet D(3,3,6);
  double t0 = 1;
  DCnTimeMap::SolutionCurve solution(t0);
  double finalTime = 0.125;
  u = tm(t0+finalTime,u,D,solution);
  check(mask,D);

  double t[] = {0.0,finalTime/3,finalTime/2,finalTime*0.75,finalTime};
  for(int i=0;i<0;++i){
    solution.eval(t0+t[i],D);
    check(mask,D);
  }
}
